...
function ArchiveRow({ tender, portalId }: { tender: ArchivedTender | ArchivedGemTender; portalId: string }) {
  ...
  const isExpiringToday = isDateToday(tender.deadline)
  const isExpired = isDatePast(tender.deadline)
        ...
        <td className="px-4 py-3 max-w-xs">
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {(tender.keywords_matched ?? []).slice(0, 3).map((kw) => (
              <span
                key={kw}
                className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded font-medium uppercase tracking-wide"
              >
                {kw}
              </span>
            ))}
          ...
          {/* Expiry status badge */}
          {isExpiringToday && (
            <span className="inline-flex items-center gap-1 mt-1.5 text-[10px] font-medium text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
              <Clock size={10} />
              Expiring Today
            </span>
          )}
          {isExpired && !isExpiringToday && (
            <span className="inline-flex items-center gap-1 mt-1.5 text-[10px] font-medium text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-200">
        ...
        <td className="px-4 py-3 text-xs text-slate-600 max-w-[160px]">
          {tender.organization ? (
            <span className="flex items-start gap-1">
              <Building2 size={11} className="mt-0.5 flex-shrink-0 text-slate-400" />
              {tender.organization}
            </span>
          ) : (
          ...
          {tender.deadline ? (
            <span
              className={clsx(
                ...
                isExpiringToday && 'text-amber-600 font-medium',
                isExpired && !isExpiringToday && 'text-red-500'
              )}
            >
              <Calendar size={11} />
...
export default function ArchivePage() {
  ...
  // Sort: expiring today first, then by deadline (closest first), then by archived date
  const sortedFiltered = useMemo(() => {
    return [...filtered].sort((a, b) => {
      const aExpiringToday = isDateToday(a.deadline)
      const bExpiringToday = isDateToday(b.deadline)

      // Expiring today come first
      if (aExpiringToday && !bExpiringToday) return -1
      if (!aExpiringToday && bExpiringToday) return 1
      ...
      // Fallback to archived date
      return parseISO(a.archived_at).getTime() - parseISO(b.archived_at).getTime()
    })
  }, [filtered])
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative">
              ...
              >
                {PORTALS.filter(p => !p.comingSoon).map((p) => (
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-slate-500" style={lufgaRegularStyle}>
              Filters:
            </span>
          </div>
          ...
          <span className="text-xs text-slate-400 ml-auto" style={lufgaRegularStyle}>
            {isLoading ? 'Loading...' : `${sortedFiltered.length} results`}
          </span>
        </div>
      </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="px-3 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wide sticky right-0 bg-slate-50 border-l border-slate-100 shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.04)]">
                    Actions
                  </th>
                </tr>
              </thead>
